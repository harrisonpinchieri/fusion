...
class Base(models.Model):
    criados = models.DateField('Criação',
                               auto_now_add=True)  # só vai ser colocado esse valor na adição quando um objeto for criado.
    modificado = models.DateField('Atualização',
                                  auto_now=True)  # quando um objeto for modificado, o valor será atualizado
...
class Features(Base):
    ICONE_CHOICES = {
        ...
        ('lni-leaf', 'Folha'),
        ('lni-layers', 'Layers'),
        ('lni-laptop-phone', 'Fone'),
        ('lni-mobile', 'Mobile'),
        ('lni-rocket', 'Foguete'),
    }
    nome = models.CharField('Nome', max_length=100)
    descricao = models.TextField('Descricao', max_length=200, default='')
